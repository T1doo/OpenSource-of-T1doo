...
- **PDF → 笔记**：将任意电子版 PDF（教材、课件、论文）转化为结构清晰的 Obsidian Markdown 笔记
...
- **智能回退**：Read tool 失败时自动切换到 pdfplumber，支持 UTF-8 编码，无缝处理中英文
- **大文件处理**：自动分块读取（Read tool ≤20 页/次，pdfplumber ≤10 页/次），支持任意大小的 PDF（无 100MB 限制）
...
用户请求（自然语言）
  ...
  │ 2. 读取 PDF (智能两层回退)    │
  │                             │
  │  Method 1: Read tool        │  最快，适合小文件 (<100MB)
  │      ↓ 失败                  │
  │  Method 2: pdfplumber       │  无大小限制，处理超大 PDF
  └──────┬──────────────────────┘
         │
         ▼
  ┌─────────────┐
...
本 Skill 使用**智能两层回退机制**处理 PDF：

1. **优先使用 Read tool**（最快）
   - 适合小文件 (<100MB)
   - 需要系统安装 pdftoppm (Poppler 工具)
   - 如果失败，自动进入下一层

2. **回退到 pdfplumber**（兼容性强）
   - 无文件大小限制
   - 纯 Python 实现，跨平台
   - 自动处理 UTF-8 编码，完美支持中英文
   - 可处理任意大小的电子版 PDF

**注意**：本 Skill 仅支持**电子版 PDF**（文字可选中复制）。如遇到扫描版/影印版 PDF，请使用 `pdf` skill 的 OCR 功能，或寻找电子版替代。
